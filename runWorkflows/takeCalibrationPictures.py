    ...
    for i in range(min(len(init_positions),len(final_positions))):
        ...
        for ix_pos, i_col in zip(x_pos_to_visit, x_hole_indices):
            ...
            for iy_pos, i_row in zip(y_pos_to_visit, y_hole_indices):
                ...
                etlcontroller.camera.changeFileName(f"GluePlate/Final_Calibration4/picture_col_{i_col}_row{i_row}_X_{x:.3f}Y_{y:.3f}Z_{z:.3f}RZ_{rz:.3f}J1_{j1:.3f}J2_{j2:.3f}J3_{j3:.3f}J4_{j4:.3f}.png")
