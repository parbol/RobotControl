...
if __name__ == "__main__":
    ...
    init_positions = [# [176.73, -485.67, 180, 28.08], # Old position not working now
                     ...
                     [158.00, -478.0, 180, 15.00],
                     [132.61, -473.97, 180, 00.02],
    ...
    for iteration in range(1):
        ...
        for i in range(min(len(init_positions),len(final_positions))):
            ...
            for ix_pos, i_col in zip(x_pos_to_visit, x_hole_indices):
                if i == 0 and i_col == x_hole_indices[0]:
                    init_y = 3
                else:
                ...
                for iy_pos, i_row in zip(y_pos_to_visit, y_hole_indices):
                    ...
                    etlcontroller.camera.changeFileName(f"Calibration_10-09-26/picture_col_{i_col}_row{i_row}_iteration{iteration}_X_{x:.3f}Y_{y:.3f}Z_{z:.3f}RZ_{rz:.3f}J1_{j1:.3f}J2_{j2:.3f}J3_{j3:.3f}J4_{j4:.3f}.png")
